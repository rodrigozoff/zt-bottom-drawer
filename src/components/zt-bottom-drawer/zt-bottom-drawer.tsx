...
export class ZTBottomDrawer {
  ...
  @Prop({ reflect: true, mutable: true }) positions: string = "close-b-10,bottom-b-200,middle-b-450,top-t-60";
  ...
  async componentDidLoad() {
    ...
    this.safeAreaTop = Number((getComputedStyle(this.el).getPropertyValue("--zt-sat") ?? "0").replace("px", ""));
    this.safeAreaBottom = Number((getComputedStyle(this.el).getPropertyValue("--zt-sab") ?? "0").replace("px", ""));
  ...
  @Prop({ mutable: true, reflect: true }) safeAreaTop: number;
  @Prop({ mutable: true, reflect: true }) safeAreaBottom: number;
      ...
      if (splitPosition.length === 3) {
        try {
          ...
          if (position.distanceTo === "TOP") {
            position.distanceToTop = position.distance + (this.safeAreaTop ?? 0);
          } else {
            position.distanceToTop = dimensionesWin.height - (this.safeAreaBottom ?? 0) - position.distance;
          }
          if (previous) {
